...
function initThemeSwitch() {
    ...
    const updateLogos = () => {
        ...
        if (lightMode) {
            ...
            siteLogos.attr('src', '/static/myapp/images/aero-logo-dark.png');
            ...
            siteLogos.attr('src', '/static/myapp/images/aero-logo.png');
...
const whatsappButton = document.getElementById('whatsapp-button');
  ...
  if (scrollPercent > 10) {
    ...
    // Hide button if scrolled above 10%
    whatsappButton.classList.add('hidden');
  }
});

  // Hide content (text + close) on click
  whatsappButton.addEventListener('click', function (e) {
    if (!whatsappButton.classList.contains('icon-only')) {
      whatsappButton.classList.add('icon-only');
      // Allow default behavior (link) only on second click
      e.preventDefault();
    }
  });
  ...
  function hideWhatsAppButton(event) {
    ...
    event.stopPropagation(); // Prevent triggering parent <a>
    whatsappButton.classList.add('hidden');
    whatsappButton.classList.remove('icon-only');
  }
